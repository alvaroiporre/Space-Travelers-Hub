...
const rocketsSlice = createSlice({
  ...
  reducers: {
    reserveRocket: (state, action) => {
      const rocketReserved = state.rockets.map((rocket) => {
        if (rocket.id !== action.payload) return rocket;
        return { ...rocket, reserved: true };
      });
      state.rockets = rocketReserved;
    },
...
export const { reserveRocket, cancelRocketReservation, selectRocket } = rocketsSlice.actions;
